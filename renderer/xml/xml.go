...
// A Config struct has configurations for the XML based renderers.
type Config struct {
	Writer    Writer
	HardWraps bool
	Unsafe    bool
	NewLineType util.NewLineType   
...
func (o *withWriter) SetXMLOption(c *Config) {
	c.Writer = o.value
}
...
// XML is an option name used in WithXML.
const optXML renderer.OptionName = "XML"

type withXML struct {
}

func (o *withXML) SetConfig(c *renderer.Config) {
	c.Options[optXML] = true
}

func (o *withXML) SetXMLOption(c *Config) {
	c.XML = true
}
...
func (o *withUnsafe) SetXMLOption(c *Config) {
	c.Unsafe = true
}
...
func (r *Renderer) renderHeading(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	n := node.(*ast.Heading)
	if entering {
		_, _ = w.WriteString("<heading level=\"")
...
func (r *Renderer) renderBlockquote(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	nl := util.ReturnNewLineString(r.NewLineType)
	if entering {
		if n.Attributes() != nil {
			_, _ = w.WriteString("<block_quote")
			RenderAttributes(w, n, BlockquoteAttributeFilter)
			_ = w.WriteByte('>')
		} else {
			_, _ = w.WriteString("<block_quote>" + nl)
		}
	} else {
		_, _ = w.WriteString("</block_quote>" + nl)
	}
...
func (r *Renderer) renderCodeBlock(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	nl := util.ReturnNewLineString(r.NewLineType)
	if entering {
		_, _ = w.WriteString("<code_block xml:space=\"preserve\">")
		r.writeLines(w, source, n)
	} else {
		_, _ = w.WriteString("</code_block>" + nl)
	}
	return ast.WalkContinue, nil
}
...
func (r *Renderer) renderHTMLBlock(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if entering {
		_, _ = w.WriteString("<html_block xml:space=\"preserve\" >")
		if r.Unsafe {
			l := n.Lines().Len()
			for i := 0; i < l; i++ {
				line := n.Lines().At(i)
			...
			_, _ = w.WriteString("<!-- raw HTML omitted -->" + nl)
		}
	} else {
		if n.HasClosure() {
			if r.Unsafe {
				...
				_, _ = w.WriteString("</html_block>" + nl )
				_, _ = w.WriteString("<!-- raw HTML omitted -->" + nl)
			}
		}
	}
...
func (r *Renderer) renderList(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if entering {
		_, _ = w.WriteString("<list type=" + ltype + " ")
		if n.IsOrdered() && n.Start != 1 {
			fmt.Fprintf(w, " start=\"%d\"", n.Start)
		}
		// TODO: checking tight or not
		_, _ := w.WriteString(" tight=\"false\"")
		// TODO: delimiter
		if n.Attributes() != nil {
...
func (r *Renderer) renderParagraph(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if entering {
		...
		_, _ = w.WriteString("</paragraph>" + nl)
	}
	return ast.WalkContinue, nil
}
...
func (r *Renderer) renderThematicBreak(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	nl := util.NewLineType(r.NewLineType)
	if !entering {
		return ast.WalkContinue, nil
	}
...
func (r *Renderer) renderCodeSpan(w util.BufWriter, source []byte, n ast.Node, entering bool) (ast.WalkStatus, error) {
	if entering {
		if n.Attributes() != nil {
			_, _ = w.WriteString("<code xml:space=\"preserve\"")
		...
		for c := n.FirstChild(); c != nil; c = c.NextSibling() {
			...
			if bytes.HasSuffix(value, '\n') {
				// Pattern "\r\n"
				if bytes.HasSuffix(value[:len(value)-1], '\r') {
					r.Writer.RawWrite(w, value[:len(value)-2])
					} 
			...
			} else if bytes.HasSuffix(value, '\r') {
				r.Writer.RawWrite(w, value[:len(value)-1])
				if c != n.LastChild() {
					r.Writer.RawWrite(w, []byte(" "))
				}
...
func (r *Renderer) renderLink(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	n := node.(*ast.Link)
	if entering {
		_, _ = w.WriteString("<link destination=\"")
...
func (r *Renderer) renderImage(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	n := node.(*ast.Image)
	_, _ = w.WriteString(`<image destination="`)
	if r.Unsafe || !IsDangerousURL(n.Destination) {
		_, _ = w.Write(util.EscapeHTML(util.URLEscape(n.Destination, true)))
	}
...
func (r *Renderer) renderRawHTML(w util.BufWriter, source []byte, node ast.Node, entering bool) (ast.WalkStatus, error) {
	...
	if r.Unsafe {
		_, _ = w.WriteString("<html_inline xml:space=\"preserve\" >")
		n := node.(*ast.RawHTML)
		l := n.Segments.Len()
		for i := 0; i < l; i++ {
